            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleShare} className="p-2 rounded-full bg-white shadow hover:shadow-md">
                    <ShareIcon />
                  </button>
                </div>
                {shareMessage && <p className="text-xs text-green-600 font-medium">{shareMessage}</p>}
              </div>
            </div>
                      ...
                      {images.length > 0 ? (
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              // Hide broken hero images; container's gray background will show instead
                        ...
                        <div key={idx} className="h-24 bg-gray-100 rounded-md overflow-hidden">
                          ...
                          >
                            <img
                              ...
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                // Hide broken thumbnail images; gray background remains visible
                                e.currentTarget.style.display = 'none';
                              }}
                            />
                          </button>
            ...
            {listing.promo && (
              <section className="mb-6">
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-green-600 text-sm mt-1">Use this code when booking to get special pricing</p>
                </div>
              </section>
            )}
              ...
              {listing.amenities && (
                <div className="mb-4">
                  ...
                  {Array.isArray(listing.amenities) ? (
                    ...
                    <p className="text-sm text-gray-700">{String(listing.amenities)}</p>
                  )}
                </div>
              )}
              ...
              {(listing.rules || listing.houseRules) && (
                <div>
                  ...
                  {Array.isArray(listing.rules || listing.houseRules) ? (
                    ...
                    <p className="text-sm text-gray-700">{listing.rules || listing.houseRules}</p>
                  )}
                </div>
              )}
            </section>
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div className="text-lg font-semibold">{listing.rate ? `₱${listing.rate}` : 'Price'}</div>
              ...
              <div className="space-y-3 mb-4">
                <div className="relative">
                  ...
                  />
                  {listing.maxGuests && (
                    <p className="text-xs text-gray-500 mt-1">Maximum {listing.maxGuests} guests</p>
                  )}
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={listing.promo ? 'Enter code (optional)' : 'No promo available'}
                  ...
                  {promoMessage && (
                    <p className={`mt-1 text-xs ${discountApplied ? 'text-green-700' : 'text-red-600'}`}>
                      {promoMessage}
                    </p>
                  )}
                </div>
                    ...
                    <PayPalButtons
                      ...
                      onApprove={async (data, actions) => {
                        try {
                          ...
                          setBookingSuccess('Payment successful! Your booking request has been sent to the host.');
                          setShowPayment(false);
                          setIsProcessing(false);
                          ...
                          setBookingError('Payment was captured, but we could not create your booking. Please contact support.');
                          setIsProcessing(false);
                        }
                      }}
                      onError={(err) => {
                        ...
                        setBookingError('There was an error processing your PayPal payment. Please try again.');
                        setIsProcessing(false);
                      }}
                      onCancel={() => {
                        setIsProcessing(false);
              ...
              <div>
                ...
                <div className="mb-3 max-h-64 overflow-y-auto border border-gray-200 rounded-md bg-gray-50 p-2">
                  {chatLoading ? (
                    ...
                    chatMessages.map((msg) => {
                      const isGuest = msg.senderRole === 'guest' || msg.guestId === user?.uid;
                      const createdAt = msg.createdAt?.toDate ? msg.createdAt.toDate() : null;
                      return (
                        <div
                          key={msg.id}
                          className={`mb-2 flex ${isGuest ? 'justify-end' : 'justify-start'}`}
                        >
                          <div
                ...
                <textarea value={messageText} onChange={(e) => setMessageText(e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-1 focus:ring-green-500" placeholder="Ask the host a question" />
                {messageError && <div className="text-red-600 text-sm mt-2">{messageError}</div>}
                <button onClick={handleSendMessage} className="w-full mt-3 px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">
                  Send Message
                </button>
              </div>
            </div>
